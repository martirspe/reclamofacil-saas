  ...
  template: `
    <div class="fixed top-6 right-6 z-50 pointer-events-none">
      @for (toast of toastService.toasts$(); track toast.id) {
        <div
          class="mb-3 pointer-events-auto toast-enter"
        >
          <div [ngClass]="getToastClasses(toast.type)" class="rounded-lg shadow-lg px-4 py-3 flex items-center gap-3 max-w-sm">
            <!-- Ícono -->
            <span class="shrink-0">
              @switch (toast.type) {
                @case ('success') {
